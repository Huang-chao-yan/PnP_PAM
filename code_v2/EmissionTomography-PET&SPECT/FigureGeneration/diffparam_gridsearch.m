...
%--------------------------------------------------------------------------
 ...
 for i = 1:100
  ...
  Cost_params.reg_choice =2;% input(' Enter 0 for Tikhonov; 1 for TV; and 2 for Laplacian. ');
  if Cost_params.reg_choice == 2;
      TVflag = 0;
     diffusion_flag =1%input(' Enter 0 for regular Laplacian and 1 for scaling computed from approximate. ');
     if diffusion_flag == 1 
       [Fxf,Fyf]=gradient(f_poisson);
       absGrad = Fxf.^2+Fyf.^2;
       thresh = absGrad.*(absGrad>epsilon*max(absGrad(:)));
       Ddiag = max(1./(1+thresh),xi);%eps^(1/2)/alpha);
...
%---------------------------------------------------------------------
  f_0 = ones(nx,ny);
 %{
 method_flag = 4;%input('Enter 1 for GCV, 2 for DP, 3 for UPRE, or 4 for no method ');
  if method_flag == 3
  ...
  %}
  Cost_params.reg_param = 10^alpha;
  cpu_t0 = cputime;
...
%---------------------------------------------------------------------
xtrue =Emission;
rel_err=norm(f_temp(:)-xtrue(:))/norm(xtrue(:));
errorvec = [errorvec; rel_err];
     end
     errorgrid = [errorgrid errorvec];
 end
 %{
