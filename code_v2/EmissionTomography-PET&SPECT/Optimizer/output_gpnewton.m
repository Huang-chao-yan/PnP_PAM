function [iter_hist] = output_gnewton(iter,gp_iter,cg_iter,stepnorm, ...
    pgradnorm,J,Active,iter_hist,x,params)
...
%    iter_hist(9,k) = relative error
  ...
  iter_hist = [iter_hist [iter; J; pgradnorm; stepnorm; n_active/ndim; ...
                TOTAL_COST_EVALS; TOTAL_HESS_EVALS; TOTAL_FFTS]];%rel_error;]];

  if params.disp_flag == 1;
    fprintf('It=%d J=%6.3e |s|=%6.3e |pg|=%6.3e #gp=%d #cg=%d #Active=%d\n',iter, J, stepnorm, pgradnorm, gp_iter, cg_iter, n_active); 
  end
